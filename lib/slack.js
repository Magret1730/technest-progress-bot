...
async function sendMessage(options = {}) {
  ...
  if (hasFormValues) {
    ...
    if (!settings.hasSavedSettings) {
      throw new Error('No saved weekly settings found. Save weekly settings on the dashboard first.');
    }
  ...
  if (!/^[CGD]/i.test(channel.trim())) {
    throw new Error('Destination ID must start with C, G, or D.');
  }
